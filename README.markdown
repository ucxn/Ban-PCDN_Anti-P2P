...
> <span style="color:#7F8C8D;">*小贴士：能在应用层解决的问题（比如用“李跳跳”去广告），就不要在三层网络层折腾。手机端看 B站建议直接上第三方客户端（如 [PiliBro](https://github.com/ucxn/Bili.Libre.Speed)），电脑端永远只用 Web 网页版并配合篡改猴插件！*</span>
...
#### 5. 禁用UDP的焦土政策 (`ACL`)
主力打手游之类的终端尝试在应用层屏蔽不必要的流氓软件（MalWare）、例如B站推荐[Libre.Speed](https://github.com/ucxn/Bili.Libre.Speed)；PC这边通常问题不大，禁用 WebRTC，多用浏览器就安全了。
...
2. 导航至 **安全 -> 防火墙 -> URL 过滤**。
3. 将模式设置为 **“黑名单”**。
4. 依次将 `PCDN域名URL关键字.xlsx` 中**黑色加粗**的关键词（如 `pcdn`, `mcdn`, `hcdn`, `p2p`）添加到 URL 过滤规则中。
...
© 2026 哥哥科技 | 遵循 CC BY-SA 4.0 协议
> **版权所有 (c) 2026 哥哥科技**
> 本作品采用 **[知识共享署名-相同方式共享 4.0 国际许可协议](https://creativecommons.org/licenses/by-sa/4.0/deed.zh-hans)** 进行许可
